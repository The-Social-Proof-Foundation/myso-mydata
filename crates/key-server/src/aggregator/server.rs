...
//! Aggregator server for MyData committee mode. It fetches encrypted partial keys from committee
...
use mydata_sdk::{FetchKeyRequest, FetchKeyResponse};
...
async fn main() -> Result<()> {
    ...
    // Spawn background task to push metrics to mydata-proxy if configured.
...
/// Spawn a metrics push background job that pushes metrics to mydata-proxy
...
async fn load_committee_state(
    ...
    let mut grpc_client =
        MySoGrpcClient::new(options.node_url()).context("Failed to create MySoGrpcClient")?;
    let key_server_v2 =
...
mod tests {
    ...
    use rand::thread_rng;
    use mydata_sdk::types::Certificate;
    use serde_json::json;
    use myso_sdk_types::UserSignature;
    use myso_types::collection_types::{Entry, VecMap as MySoVecMap};
    use wiremock::{
    ...
    fn create_test_app_state(
        ...
        let registry = Registry::new();
        let metrics = Arc::new(AggregatorMetrics::new(&registry));
        let grpc_client = MySoGrpcClient::new(options.node_url()).unwrap();

        AppState {
            aggregator_metrics: metrics,
            grpc_client,
            threshold: Arc::new(RwLock::new(threshold)),
            committee_members: Arc::new(RwLock::new(VecMap(MySoVecMap {
