...
description: Learn security best practices for using MyData, including threshold configuration, key server selection, encryption strategies, and risk mitigation approaches.
keywords:
...
The MyData SDK's `encrypt` API returns a symmetric key used to encrypt your data. If you decide to keep this key - for example, to support disaster recovery - store it securely and follow strict security practices. Alternatively, you can return the key to the user instead of storing it yourself. In that case, the user must take responsibility for securely managing the key to prevent any leaks.
