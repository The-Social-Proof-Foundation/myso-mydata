...
| `CONFIG_PATH` | `/app/config/key-server-config-railway.yaml` | Config mount path |
...
4. **Config file:** Add a volume named `config` at `/app/config` and provide `key-server-config-railway.yaml` for each service
5. **Health check:** `GET /v1/service`
...
`MASTER_KEY` must be set as a **secret** in Railway, not in the config file. The config file must be provided via the volume mount.
...
1. **Health:** `curl https://<key-server-url>.up.railway.app/v1/service`
...
| Health check fails | Wrong port | Key server uses `PORT` env; ensure `/v1/service` is exposed |
