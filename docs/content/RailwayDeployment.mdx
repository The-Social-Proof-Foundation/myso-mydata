...
- **Storage:** Uses filesystem (volume at `/tmp/mimir`). Mount a volume at `/tmp/mimir` before deploy.
...
| Mimir deploy "failed" despite logs showing "Application started" | Health check port mismatch | Ensure Mimir listens on `PORT` (set to 9009); Railway health checks hit `PORT`; verify service Port/Target Port in Railway dashboard matches 9009 |
