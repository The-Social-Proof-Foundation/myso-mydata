...
| `NODE_URL` | `http://fullnode.testnet.mysocial.network:9000` | Override if needed |
...
- **Storage:** Uses Google Cloud Storage (GCS). Before deploy:
  1. Create GCS buckets: `mimir-blocks`, `mimir-ruler`
  2. Create a GCP service account with Storage Object Admin on those buckets
  3. Add Railway variable `GCP_SERVICE_ACCOUNT_JSON` with the full service account JSON
  4. Mount a volume at `/tmp/mimir` for local TSDB and activity log
  5. **JSON format:** Use minified (single-line) JSON to avoid "invalid character '-' in numeric literal". No YAML list markers (`-`), no extra quotes. Validate at jsonlint.com before pasting.
...
| mydata-proxy starts with `sed`/`url:.*` errors | Stale image or wrong Dockerfile path built | Ensure proxy service builds `Dockerfile.mydata-proxy`, keep `startCommand` as `/opt/mydata-proxy/entrypoint.sh`, then trigger a fresh deploy |
...
| Mimir "invalid character '-' in numeric literal" | Malformed `GCP_SERVICE_ACCOUNT_JSON` | Use minified single-line JSON; no YAML markers; validate at jsonlint.com |
